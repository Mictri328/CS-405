...

{
  ...
  //initialize the empty string to store the line read from file
  string str;
  //open file streamm to read from file
  ifstream file(filename);
   while (getline(file, str))
    {
        //add new line after each line and append to the final string
        file_text += str+"\n";
...

{
  ...
  time_t n = time(0);
   ...
   char* dat = ctime(&n);
   //open filestream to store the data to file
...
int main()
{
  ...
  // Lines 3+: <lorem ipsum generated with 3 para graphs> 
