...
TEST_F(CollectionTest, VerifyResizingIncreases)
{
    ...
    // assert size must be 5
    ASSERT_EQ(collection->size(), 5);

    // resize to 20
    collection->resize(20);

    // assert new size must be 20
...
TEST_F(CollectionTest, EraseClearsCollection)
{
    ...
    // erase the collection from begining to end
...
TEST_F(CollectionTest, addElement) {

    add_entries(10);

    // Adds one element, in this case it adds an element with a value of 1
    collection->push_back(1); 

    ASSERT_TRUE(collection->size() > 10);

    // 11th element is index 10, so we check index 10 to make sure its equal to 1
    ASSERT_TRUE(collection->at(10) == 1); 
}

TEST_F(CollectionTest, removeElement) {
