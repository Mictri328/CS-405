...

{
    ...
    {
        if (result > INT_MAX - increment) //checking the overflow condition 
        {
            std::cout << "Overflow previous step "  << std::endl;
            std::cout << "Previous result is :" << result << std::endl;     //indicating that addition failed
            break;      //preventing the overflow
        }
        result += increment;
    }

    return result;  //returning the correct value 
}
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    {
        if (result < INT_MIN + decrement) //checking the underflow condition 
        {
            std::cout << "Underflow previous step " << std::endl;
            std::cout << "Previous result is :" << result << std::endl;     //indicating that subtraction failed 
            break;      //preventing the underflow
        }
        result -= decrement;
    }
